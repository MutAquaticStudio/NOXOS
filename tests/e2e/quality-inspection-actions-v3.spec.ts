...
for (const width of [1440, 320])
  test(`Inspection confirmations at ${width}`, async ({ page }) => {
  ...
  `
    ...
    await page.goto("/quality-control/inspections/initial");
    ...
    for (const action of ["Finalize", "Cancel", "Reinspect"]) {
      await page.evaluate((action) => {
        (window as any).statusValue = action === "Reinspect" ? "FINAL" : "DRAFT";
        (window as any).go("/quality-control/inspections/" + action);
      }, action);
      await page.getByRole("button", { name: action, exact: true }).click();
      const dialog = page.getByRole("dialog", { name: "Confirm inspection action" });
      await expect(dialog).toContainText("QC-INSPECT");
      ...
      if (action === "Reinspect") {
        ...
        expect(await page.evaluate(() => (window as any).writes.length)).toBe(count);
        await page.getByLabel("Retest reason").fill("Controlled retest");
      }
      await dialog
      ...
      expect(await page.evaluate(() => (window as any).writes.at(-1))).toMatchObject({
        path: "/quality-control/inspections/" + action + "/" + action.toLowerCase(),
        tenantId: "tenant-a",
        method: "POST"
      });
