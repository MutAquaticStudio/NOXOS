...
const g13VisualCaptureDirectory = process.env.G13_VISUAL_CAPTURE_DIR;
...
try {
  ...
  for (const tenantId of [tenantA, tenantB]) {
    expectStatus(
      await api(
        platformOwnerToken,
        `/platform/tenants/${tenantId}/entitlements/module.commercial-orders`,
        { method: "PUT", body: { enabled: true } }
      ),
      200,
      "Commercial Orders entitlement"
    );
  }
  ...
  try {
    ...
    await refreshToken("B");
    await runG13StagingAcceptance(page, tenantA, tenantB);
  } finally {
    await browser.close();
  }
...
async function runG13StagingAcceptance(
  page: Page,
  tenantA: string,
  tenantB: string
): Promise<void> {
  ...
  expectError(
    await api(token("E"), `/commercial-orders/orders/${orderId}`, { tenantId: tenantB }),
    404,
  ...
  await signInInBrowser(page, fixture("B"));
  await page.goto(new URL("/commercial-orders", stagingUrl).toString(), {
    waitUntil: "networkidle"
  });
  await expectHeading(page, "Commercial Orders");
  await captureG13(page, "commercial-orders-registry-desktop", "/commercial-orders");
  await page.goto(new URL(`/commercial-orders/${orderId}`, stagingUrl).toString(), {
    waitUntil: "networkidle"
  });
  await expectHeading(page, orderNumber);
  await captureG13(page, "commercial-orders-detail-desktop", `/commercial-orders/${orderId}`);
  await signOutInBrowser(page);
  console.log("G13_STAGING_COMMERCIAL_ORDERS_ACCEPTANCE=PASS");
  console.log("G13_STAGING_G7_COMMERCIAL_PROVENANCE=PASS");
  console.log("G13_STAGING_G10_RELEASE_REVALIDATION=PASS");
  console.log("G13_STAGING_G11_SERVICE_REVALIDATION=PASS");
  console.log("G13_STAGING_TENANT_SECURITY=PASS");
}
